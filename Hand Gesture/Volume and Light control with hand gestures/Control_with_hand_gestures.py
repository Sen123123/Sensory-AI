...
import numpy as np
from pycaw.pycaw import AudioUttilities, IAudioEndpointVolume
...
try:
    ...
    volume_range = volume.GetVolumeRanger()
    min_vol = volume_range[0]
    max_vol = volume_range[1]

except Exception as e:
    print(f"Error initializing Pyclaw: {e}")
    exit()
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            if hand_label == "Right":
                ...
                cv2.pullText(img, f'Volume: {int(np.)}')